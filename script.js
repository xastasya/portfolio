/* ============================================================
   ...
   Версия: полностью исправленная, с вертикальным модалом
   ============================================================ */
...
const IMAGES = [
  ...
  'assets/images/p1_4.png'
];

// 15 проектов, у каждого минимум 5 изображений
// script.js (фрагмент с изменениями)
const PROJECTS = [
  { id:0, title:'Твич оформление YaJeBarracuda', cat:'stream', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Стрим', desc_en:'Stream', year:'2025', tools:'Illustrator, Photoshop', type_ru:'Брендинг', type_en:'Branding', 
    thumbnail: 'assets/images/p0.png', // ← обложка только для сетки
    images:[
  ...
  { id:1, title:'Твич оформление Lane', cat:'stream', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Стрим', desc_en:'Stream', year:'2025', tools:'Illustrator', type_ru:'Брендинг', type_en:'Branding', 
    thumbnail: 'assets/images/l0.png', // ← обложка только для сетки
    images:[
...
{ id:2, title:'Твич оформление BlackRaven', cat:'stream', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Стрим', desc_en:'Stream', year:'2026', tools:'Figma, Illustrator', type_ru:'Брендинг', type_en:'Branding', 
  thumbnail: 'assets/images/r0.png', // ← обложка только для сетки
  images:[
  ...
  'iframe:https://kinescope.io/embed/5jpkQ5C198VLbZAnVq7X6K'
] },
  { id:3, title:'Твич оформление Runoka1e', cat:'stream', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Стрим', desc_en:'Stream', year:'2025', tools:'Figma, Illustrator', type_ru:'Брендинг', type_en:'Branding', 
    thumbnail: 'assets/images/k0.png', // ← обложка только для сетки
    images:[
...
{ id:4, title:'Твич оформление The Salem Cat', cat:'stream', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Стрим', desc_en:'Stream', year:'2026', tools:'Figma', type_ru:'Брендинг', type_en:'Branding',
  thumbnail: 'assets/images/s0.png', // ← обложка только для сетки
  images:[
...
thumbnail: 'assets/images/t0.png', // ← обложка только для сетки
  images:[
...
{ id: 25,
  ...
  thumbnail:'assets/images/li1.png',   // обложка для сетки
  images:[
...
{ id:27, title:'Яндекс продвижение', cat:'graphic', catName:'Graphic Design', catName_ru:'Graphic Design', desc_ru:'Иллюстрации для интерфейса «Простой монетизации». Для оформления преимуществ сервиса был разработан сет из четырех иллюстраций. В основе — метафорический подход: Ракета и шестерня - запуск сложных настроек. Свинья-копилка - аккумулирование выплат. Телефонная трубка - поддержка и легкая интеграция. Смартфон и графики - аналитика в одном кабинете. Использование ярких акцентов (лаймовый цвет) на глубоком фиолетовом фоне помогает выделить ключевые объекты и направляет взгляд пользователя при скролле.', desc_en:'A set of four illustrations developed to showcase service benefits. The metaphors include: a rocket and gear for launching complex settings, a piggy bank for accumulating payouts, a telephone handset for support and easy integration, and a smartphone with charts for all-in-one analytics. Bright lime accents on a deep purple background highlight key objects and guide the users eye while scrolling.', year:'2024', tools:'Illustrator, Photoshop', type_ru:'Реклама', type_en:'Advertising', images:[
  ...
  images: [
    ...
    'iframe:https://kinescope.io/embed/rAEduPFVpyiJiTjB8o43Qe:100%'   // ← исправлено
  ]
},
{ id: 29,
  title:'Makillian',
...
{ id: 35,
  ...
  type_ru:'Айдентика',
  type_en:'Identity',
  thumbnail:'assets/images/sh1.png',  // обложка в сетке
  images:[
...
{ id: 40,
  ...
  type_ru:'Фотографии',
  type_en:'Photos',
  thumbnail:'assets/images/po1.png',   // обложка для сетки
  images:[
...
function renderOldModalImage() {
  ...
  // Удаляем старый iframe-контейнер, если он есть
  const oldIframeContainer = modalMainImg.parentNode.querySelector('.iframe-container');
  if (oldIframeContainer) oldIframeContainer.remove();
  
  if (isIframe) {
    modalMainImg.style.display = 'none';
    let iframePart = media.slice(7);
    let paddingPercent = '56.25%';
    ...
    // Текст-заглушка "Загрузка..." на светлом фоне
    ...
    loader.style.backgroundColor = '#f4f6f8'; // светлый фон
    loader.style.color = '#9098AE';           // акцентный цвет
    ...
    // Загрузка длится ровно 1 секунду
    setTimeout(() => {
...
function applyLanguage(lang) {
  ...
  // Все элементы с data-ru / data-en
  ...
  // Обновить категории в карточках проектов
  ...
  // Если открыт старый модал — обновить его содержимое
  ...
  // Обновить подсказку в вертикальном модале
  updateScrollHintText();
}
...
function initAccordion() {
  ...
  // Убираем все активные классы (их больше нет)
  accItems.forEach(item => {
    item.classList.remove('active');
    // Обработчик клика для открытия модала (без изменения)
...
(function initCursorGlow() {
  ...
  // Проверка на тач-устройства (отключаем, чтобы не мешало)
  ...
  function updateGlow() {
    // Плавное следование: чем меньше число (0.08), тем мягче отставание
  ...
  // Скрываем свечение, когда мышь покидает окно
...
if (grid) {
  document.addEventListener('mousemove', (e) => {
    const x = (e.clientX / window.innerWidth - 0.5) * 20;   // движение по X
    const y = (e.clientY / window.innerHeight - 0.5) * 12;  // движение по Y
    grid.style.transform = `translate(${x}px, ${y}px)`;
  });
}
// Плавная анимация полосок навыков (старт с 0, расширение до target)
function animateSkillBars() {
  ...
  fills.forEach(fill => {
    ...
    if (targetWidth && !fill.classList.contains('animated')) {
      // Убедимся, что ширина сейчас 0
      fill.style.width = '0';
      // Небольшая задержка, чтобы браузер зафиксировал 0
      setTimeout(() => {
...
// Запускаем анимацию, когда секция резюме видна
...
if (resumeSection) {
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        animateSkillBars();
        observer.disconnect(); // анимация срабатывает только один раз
      }
    });
  }, { threshold: 0.2 });
  observer.observe(resumeSection);
}
// Функция, которая оборачивает & в span.amp
function wrapAmpersand(element) {
  if (!element) return;
  // Заменяем " & " на " <span class='amp'>&amp;</span> " (с пробелами)
  // Учитываем как &, так и &amp; (HTML-сущность)
...
// При загрузке страницы обрабатываем .hero-subtitle
...
function scrollToActiveThumb() {
  ...
  // Вычисляем, насколько нужно прокрутить, чтобы миниатюра оказалась по центру
